...
#define DOWNCAST(SINK) container_of(SINK, struct sc_streamer, frame_sink)
static bool
sc_decoder_open(struct sc_streamer* streamer, AVCodecContext* ctx) {
    streamer->frame = av_frame_alloc();
    if (!streamer->frame) {
        LOG_OOM();
        return false;
    }
